...
## 📅 2026-03-23 — ABSA 프롬프트 v4 적용 + 이메일 생성 프롬프트 개선

### ✅ ABSA 프롬프트 v4 적용 (`analyze` Edge Function)

**배경**
- 기존 프롬프트: 통합 기준 없이 분리 규칙만 있어 원인-결과까지 과세분화되는 문제
- 두 논문(한국어 ABSA, 패션/휴대폰 리뷰) 인사이트 반영
- 설계 고민 → `docs/absa-prompt-comparison.md` 섹션 9·10에 기록

**주요 변경**
- 역할 부여: HRD/온보딩 경험 분석 전문가
- 한국어 해석 원칙 7가지 명시 (역설 표현, 유보적 긍정, 이중부정 등)
- **분리 규칙** (해결 주체 다름 / 독립 액션 / 병렬 대상 / 감성 극성 다름)
- **통합 규칙** (원인-결과 / 동일 행동 재표현 / 파생 효과 / 반복 동일 감성) — 기존 MD의 핵심 빈칸 해소
- `confidence` 기준 명시 (높음/보통/낮음)
- `sentiment_score` 출력 제거 → 후처리 매핑으로 전환
- `rationale_short` 필드 추가 (짧은 판단 근거 한 문장)
- Few-shot 예시 5개 (경계 사례 중심: 원인-결과 통합, 동일 행동 통합, 역설 표현, 해결 주체 분리, 유보적 긍정)

**UI 변경**
- `AdminSurvey.js`: ABSA 결과 테이블에서 "점수" 컬럼 제거

### ✅ 이메일 생성 프롬프트 개선 (`generate-email` Edge Function)

**배경**
- 기존: `analysisResult.aspects` 전체를 그대로 넣고 톤 가이드 중심 생성
- 문제: 누가 해결할지(owner), 우선순위, 반복 신호 해석이 프롬프트에서 통제 안 됨
- "예쁘게 쓰기"는 됐지만 "정확하게 넛지하기"는 약한 구조
- 설계 고민 → `docs/absa-email-prompt-design.md` 신규 생성

**멘토용 프롬프트 핵심 변경**
- 강점 최대 1개 + 보완 포인트 최대 1개만 선택
- owner=mentor 항목 우선 선별, 예산/비품/제도 제외
- 불릿 포인트 금지, 3~4문단 서술형
- 행동 제안을 1~2주 내 실천 가능한 구체적 문장으로
- quote 직접 인용 금지 → 완곡한 요약 표현으로 전환
- 신입/경력 구분은 `employeeLabel` 변수로 프롬프트 내 처리 (toneGuide 4분기 제거)

**팀장용 프롬프트 핵심 변경**
- owner=team|organization|environment|program 우선 선별
- 유지 포인트 1개 + 함께 살펴볼 포인트 최대 2개
- 전반 상태 한 줄 요약 → 불릿 3개(유지/살펴볼/팀 차원 제안) 구조
- HR/제도/예산 이슈는 팀장 책임 아닌 "참고/협업 필요"로 표현
- 감성 분포를 숫자가 아닌 질적 요약으로

**구조 변경**
- 기존: `toneGuide` 4분기 변수 → `prompt` 조립
- 변경: `recipient_type === 'mentor'` 삼항 연산자로 프롬프트 직접 분기

### 📄 문서 추가

| 파일 | 내용 |
|------|------|
| `docs/absa-prompt-comparison.md` | 섹션 9 (few-shot 설계 및 논문 인사이트), 섹션 10 (v4 최종 프롬프트) 추가 |
| `docs/absa-email-prompt-design.md` | 이메일 프롬프트 설계 고민 신규 생성 (멘토/팀장 최종 프롬프트 포함) |

### 커밋 이력

| 커밋 | 내용 |
|------|------|
| `e1840a8` | docs: ABSA few-shot 설계 및 v3 프롬프트 개선안 추가 |
| `85347b7` | docs: ABSA 최종 프롬프트 v4 추가 (섹션 10) |
| `55a1478` | feat: ABSA 프롬프트 v4 적용 및 점수 컬럼 제거 |
| `dbdfa39` | docs: 이메일 생성 프롬프트 설계 고민 문서 추가 |
| `13629cf` | docs: 멘토/팀장 메일 생성 프롬프트 최종본 추가 (섹션 10) |
| `d838f74` | docs: 이메일 프롬프트 설계 문서 미결 항목 제거 |
| `069c2f6` | feat: 메일 생성 프롬프트 개선 (멘토/팀장 역할 기반 넛지 구조로 교체) |

---

## 📅 2026-03-27 — UI 전면 리디자인 (로그인 + 사용자/관리자 대시보드)

### ✅ 시스템 명칭 변경
- 앱 이름: `YURA 멘토링 관리 시스템` → **`YURA 온보딩 시스템`**
  - `public/index.html` `<title>` 변경 (크롬 탭 표시)
  - `AdminLayout.js` 사이드바 로고 텍스트 변경
  - 관리자 메뉴 레이블: `멘토링 현황` → `온보딩 현황`

---

### ✅ 로그인 화면 전면 개편 (Google Stitch 디자인 참고)

**레이아웃 구조**
- 2단 레이아웃: 좌(히어로 영역, 8fr) + 우(폼 영역, 4fr)
- 히어로 배경: 파란 그라데이션 `#344dbe → #4f67d8`

**좌측 히어로**
- "YURA 온보딩 시스템" 타이틀
- "유라코퍼레이션 입사를 진심으로 환영합니다!" 헤드라인 (줄간격 확대)
- 온보딩 프로그램 / 멘토 매칭 소개 카드 2개

**우측 폼**
- YURA 로고 이미지 (컬러 로고, 가운데 정렬)
- 서브타이틀: "로그인하여 시작하세요."
- 사번·비밀번호 입력 필드 (아이콘 포함)
- 초기 비밀번호 안내 + 문의처 (인사기획팀 박상혁 선임, 1456)
- 저작권: © 2026 YURA Corporation

**폰트/크기 조정**
- 서브타이틀, 라벨, 힌트/문의 텍스트 1.5배 확대
- 모바일 반응형 `@media (max-width: 768px)` 추가

---

### ✅ 사용자 대시보드 개편 (`MainMenu.js` / `MainMenu.css`)

- 헤더 색상: 보라 그라데이션 → 파란색 `#344dbe`
- 카드 테두리: 컬러별 → 파란색 계열 통일
- 환영 문구 폰트 1.5배 확대
- 서브타이틀: "멘토링 과정을 시작하세요" → "온보딩 과정을 시작하세요!"
- 스크롤 없이 화면 내 배치 (flex 높이 조정)
- 모바일 반응형 레이아웃 추가
...
#### AdminLayout (사이드바 + 상단바)

**사이드바**
- 배경: `#1a2332` (다크 네이비, 기존 유지)
- 활성 메뉴: 파란 tint `rgba(255,255,255,.18)`
- 호버: `rgba(255,255,255,.10)`

**상단바 신규 추가** (`admin-topbar`)
- 현재 페이지 제목 자동 표시 (메뉴 전환 시 동적 변경)
- 우측: 🔔 알림 + ⚙️ 설정 아이콘
- 배경: 화이트, 하단 border `#e8eaf0`

**본문 페이지 제목 제거**
- 5개 페이지의 `<h1 className="page-title">` 제거
  - AdminOnboarding, AdminAnnouncements(×2), AdminSurvey, AdminUsers, AdminMentorBuddy
...
| 항목 | 변경 내용 |
|------|-----------|
| 설문조사 버튼 | `white-space: nowrap` 추가 → 모바일에서 줄바꿈 방지 |
| 멘토 관리 테이블 | `멘토/버디` → `멘토`, `멘토id` → `멘토 사번` |
| 멘토 지정 팝업 | `멘토/버디 지정` → `멘토 지정` |
| 설문조사 관리 | 이름 th 가운데 정렬, 이름/차수 정렬 기능 추가 |
...
| 커밋 | 내용 |
|------|------|
| `bef94db` | design: 관리자 사이드바 Stitch 스타일 적용 |
| `0bdb560` | design: 관리자 사이드바 Stitch 남색(#344dbe) 적용 |
| `28c4bd1` | design: 관리자 Stitch 스타일 상단바 추가 + 사이드바 #1a2332 원복 |
| `f45d43c` | design: 관리자 각 페이지 본문 내 제목 제거 |
| `feaeffd` | fix: 공지사항 관리 본문 제목 제거 누락 수정 |
| `d353578` | design: 상단바 우측 유저 정보 제거, 알림/설정 아이콘만 유지 |
| `4714e15` | fix: 멘토 관리 - 멘토id→사번, 멘토/버디→멘토 텍스트 변경 |
| `d265bc1` | fix: 멘토 관리 테이블 헤더 사번→멘토 사번 |
| `5fb9d03` | feat: 설문조사 관리 이름 th 가운데정렬 + 이름/차수 정렬 추가 |
